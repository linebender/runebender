...
impl Path {
    ...
    /// Interpret the path as a spline and return that bezier.
    ///
    /// This is hacky for sure.
    pub fn spline_bezier(&self) -> BezPath {
        let bez = self.bezier();
        ...
        for el in bez.elements() {
            match el {
                ...
                &PathEl::CurveTo(p1, p2, p3) => {
                    // Reinterpret as hyperbezier
                    let p = last_p.unwrap();
                    let v = p3 - p;
                    // This takes (0, 0) to p and (1, 0) to p3.
                    let a = Affine::new([v.x, v.y, -v.y, v.x, p.x, p.y]);
                    let a_inv = a.inverse();
                    let hb = HyperBezier::solve(a_inv * p1, a_inv * p2);
                    let curve = hb.render(8);
